...
Resolución de la máquina "Encrypt", logrando acceso inicial mediante la explotación de credenciales filtradas por error en el campo `commonName` de un certificado SSL. La escalada de privilegios a *root* se consiguió abusando de *Capabilities* (`cap_setuid=ep`) mal configuradas en el intérprete de Ruby.

**Herramientas utilizadas:** `arp-scan`, `nmap`, `getcap`, `ruby`.
...
Ejecutamos `ls -la` para ver los archivos:
```bash
ls -la
```
...
Nos pide la contraseña (que es Goat123!); sin embargo, no tenemos permiso para usar sudo.

A veces, los usuarios dejan contraseñas o comandos sensibles en el historial de su consola. Lo revisamos con:
```bash
cat .bash_history
```
...
Algunos programas necesitan permisos de root para funcionar temporalmente. A veces, los administradores le dan esos permisos a programas que no deberían tenerlos, lo que nos permite abusar de esto. Usamos: 
```bash
...
Todos los binarios SUID listados son los que vienen por defecto en cualquier instalación de Debian.
...
Solo nos sale el `index.html` genérico.
